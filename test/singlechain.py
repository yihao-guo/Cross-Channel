...
class SingleChain():
        ...
        for server_ip in self.ips:
            ...
            for node in self.nodes:
                if node.ip == server_ip:
                    ...
                    command = 'docker cp %s:/home/u0/proof_need/file.txt file.txt' % (node.name)
                    t = threading.Thread(target=server_ip.exec_command, args=(command,))
                    t.start()
                    threads.append(t)
            ...
            subprocess.run(['sshpass -p %s scp %s@%s:%s %s' % (self.password, self.username, server_ip.address,
                                                               'evs-test85*', '/home/u0/pridata/outfile')],
    ...
    def run_geth_nodes(self):
        ...
        for node in self.nodes:
            start_geth_command = (
                                     'geth --experiment.output=/home/u0/proof_need/file.txt  --datadir abc --networkid 55661 --port 30303 --http --http.addr 0.0.0.0 --http.port 8545 --http.api '
...
def test_get_mul_contractaddr(contract_num, nodes):
    ...
    for t in threads:
        try:
            contractaddr = t.get_result()  # contractaddr
        except:
            contractaddr = "0x1"
...
def test_send_mul_mint(mint_num, nodes, accounts, test_nodes):
    ...
    for t in threads:
        try:
            mint_hash, consensus_time, exec_time = t.get_result()  
        except:
            mint_hash = "0x1"
            consensus_time = 0
...
def test_send_mul_convert(convert_num, nodes, accounts, test_nodes):
    ...
    for t in threads:
        try:
            convert_hash, t1, t2 = t.get_result()  
        except:
            convert_hash = "0x1"
            # t1 = 0
            # t2 = 0
...
def test_send_mul_commit(commit_num, nodes, accounts, contractaddr, test_nodes):
    ...
    for t in threads:
        try:
            commit_hash, t1, t2 = t.get_result() 
        except:
            commit_hash = "0x1"
            consensus_time = 0
            exec_time = 0
...
def test_send_mul_refund(refund_num, nodes, accounts, contractaddr, test_nodes):
    ...
    for t in threads:
        try:
            refund_hash, t1, t2 = t.get_result() 
        except:
            refund_hash = "0x1"
            # consensus_time = 0
            # exec_time = 0
...
def test_send_mul_deposit(deposit_num, nodes, accounts, N, test_nodes):
    ...
    for t in threads:
        try:
            deposit_hash, t1, t2 = t.get_result()  
...
if __name__ == "__main__":
    ...
    # 
    print(nodes_A[0].ip.address)
    print(nodes_A[1].ip.address)
    print("\n")
    print(nodes_A[0].rpc_port)
    w3 = Web3(Web3.HTTPProvider("http://%s:%d" % (nodes_A[0].ip.address, nodes_A[0].rpc_port)))
    #w32 = Web3(Web3.HTTPProvider("http://%s:%d" % (nodes_A[1].ip.address, nodes_A[1].rpc_port)))
    user = w3.eth.accounts[0]
    #user2 = w32.eth.accounts[0]
    print(user)
    #print(user2)
    w3.geth.personal.unlockAccount(user, 'root')
    #w32.geth.personal.unlockAccount(user2, 'root')
